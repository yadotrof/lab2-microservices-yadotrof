# Template for RSOI lab 2 Miroservices

![Build project](https://github.com/bmstu-rsoi/lab2-microservices-yadotrof/workflows/Build%20project/badge.svg)

## Взаимодействие сервисов
```mermaid
graph TD
  A[Store Service]--> B[Order Service]
  A[Store Service] --> C[Warehouse Service]
  A[Store Service] --> D[Warranty Service]
  B[OrderService] --> D[Warranty Service]
  B[OrderService] --> C[Warehouse Service]
  C[Warehouse Service] --> D[Warranty Service]
```
![Mermaid](https://mermaid.ink/img/eyJjb2RlIjoiZ3JhcGggVERcbiAgQVtTdG9yZSBTZXJ2aWNlXS0tPiBCW09yZGVyIFNlcnZpY2VdXG4gIEFbU3RvcmUgU2VydmljZV0gLS0-IENbV2FyZWhvdXNlIFNlcnZpY2VdXG4gIEFbU3RvcmUgU2VydmljZV0gLS0-IERbV2FycmFudHkgU2VydmljZV1cbiAgQltPcmRlclNlcnZpY2VdIC0tPiBEW1dhcnJhbnR5IFNlcnZpY2VdXG4gIEJbT3JkZXJTZXJ2aWNlXSAtLT4gQ1tXYXJlaG91c2UgU2VydmljZV1cbiAgQ1tXYXJlaG91c2UgU2VydmljZV0gLS0-)

## Общая конфигурация

Все сервисы имеют health-чеки:
* https://shmarov-rsoi-lab2-store.herokuapp.com/ht/
* https://shmarov-rsoi-lab2-order.herokuapp.com/ht/
* https://shmarov-rsoi-lab2-warehouse.herokuapp.com/ht/
* https://shmarov-rsoi-lab2-warranty.herokuapp.com/ht/

Для каждого модуля отдается OpenAPI спецификация в формате json, для просмотра можно использовать Swagger UI
* https://shmarov-rsoi-lab2-store.herokuapp.com/api/v1/store/openapi
* https://shmarov-rsoi-lab2-order.herokuapp.com/api/v1/orders/openapi
* https://shmarov-rsoi-lab2-warehouse.herokuapp.com/api/v1/warehouse/openapi
* https://shmarov-rsoi-lab2-warranty.herokuapp.com/api/v1/warranty/openapi

# Запуск проектов через Docker Compose
```shell script
docker-compose up -d --build
```

## Тестирование
Для проверки работоспособности системы используются скрипты Postman.
В папке [postman](postman) содержится [коллекция запросов](postman/postman-collection.json) к серверу и два enviroment'а:
* [local](postman/postman-local-environment.json);
* [heroku](postman/postman-heroku-environment.json).

Для автоматизированной проверки используется [GitHub Actions](.github/workflows/main.yml), CI/CD содержит шаги:
* линтер;
* деплой _каждого_ приложения на Heroku;
* интеграционные тесты;
* прогон скриптов postman через newman для enviroment'а herkou.
